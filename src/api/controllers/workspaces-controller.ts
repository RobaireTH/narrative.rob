...
const workspace_run_file_params_schema = workspace_id_params_schema.extend({
  fileName: z.enum(['execution_plan', 'logs', 'narrative', 'portfolio']),
...
export function createWorkspacesController(
  ...
  return {
    ...
    async getWorkspaceRunFile(req: Request, res: Response) {
      const auth_user = getAuthenticatedUser(res);
      const { fileName, runId, workspaceId } =
        workspace_run_file_params_schema.parse(req.params);
      const file =
        await params.services.workspace_bootstrap_service.getWorkspaceRunFile({
          file_name: fileName,
          owner_id: auth_user.uid,
          run_id: runId,
          workspace_id: workspaceId,
